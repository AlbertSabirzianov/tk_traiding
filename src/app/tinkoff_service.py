...
class TkBroker:
    ...
    def close_long_positions_by_figi(self, figi: str) -> None:
        ...
        Метод проверяет, была ли уже закрыта позиция с данным FIGI, чтобы избежать повторных операций.
        Получает текущие позиции, находит позицию с указанным FIGI и положительным балансом (лонг).
        Рассчитывает количество лотов для продажи, исходя из баланса и размера лота.
        Отправляет рыночный ордер на продажу для закрытия позиции.
        Добавляет FIGI в список уже закрытых, чтобы не закрывать повторно.
    ...
    def close_short_positions_by_figi(self, figi: str) -> None:
        """
        Закрывает короткие позиции (шорты) по-заданному FIGI.

        Метод проверяет, была ли уже закрыта позиция с данным FIGI, чтобы избежать повторных операций.
        Получает текущие позиции, находит позицию с указанным FIGI и отрицательным балансом (шорт).
        Рассчитывает количество лотов для покупки, исходя из абсолютного значения баланса и размера лота.
        Отправляет рыночный ордер на покупку для закрытия короткой позиции.
        Добавляет FIGI в список уже закрытых, чтобы не закрывать повторно.
