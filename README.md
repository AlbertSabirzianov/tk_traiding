...
| moving_average                     | Система рекомендаций, на основе пересечений скользящих средних (EMA).  Для каждого тикера из списка анализирует последние значения EMA и генерирует торговые сигналы: - Покупка (BUY), если короткая EMA пересекает длинную EMA снизу вверх.- Продажа (SELL), если короткая EMA пересекает длинную EMA сверху вниз |
| moving_average_5_min               | Система рекомендаций, на основе пересечений скользящих средних (EMA) по 5 минутным свечам.                                                                                                                                                                                                                         |
| moving_average_5_min_only_by_trend | Система рекомендаций, на основе пересечений скользящих средних (EMA) по 5 минутным свечам, вход в сделку только по тренду.                                                                                                                                                                                         |
| logistic_model                     | Система рекомендаций, на основе логистической регрессии, для использования этой системы необходимо для начала создать модели с помощью скрипта ml_models_creation.py                                                                                                                                               |
# Скрипт для создания моделей логистической регрессии
В папке ```srs``` проекта кроме основного скрипта ```main.py``` так же находится скрипт по созданию моделей 
Логистической регрессии на основе исторических данных и технического анализа ```ml_models_creation.py```.
Чтобы его запустить необходимо набрать в терминале 
```commandline
python ml_models_creation.py
```
Скрипт создаст в папке DATA Модели логистической регрессии для каждого инструмента, после чего вы сможете использовать
рекомендательную систему ```logistic_model```
